...
class LocalStorageTodosApi extends TodosApi {
  ...
  Future<void> saveTodo(Todo todo) async {
    ...
    final todoIndex = await todos.indexWhere((t) => t.id == todo.id);
    if (todoIndex >= 0) {
  ...
  Future<void> deleteTodo(String id) async {
    final todos = [..._todoStreamController.value];
    final todoIndex = await todos.indexWhere((t) => t.id == id);
    if (todoIndex == -1) {
  ...
  Future<int> clearCompleted() async {
    final todos = [..._todoStreamController.value];
    final completedTodosAmount = await todos.where((t) => t.isCompleted).length;
